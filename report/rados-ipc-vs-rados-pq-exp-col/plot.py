...
if __name__ == "__main__":
    ...
    with open('vanilla-pq-16.json') as json_file:
    ...
    with open('rados-pq-16.json') as json_file:
        data_dict = json.load(json_file)
        print(data_dict)
    ...
    with open('vanilla-ipc-16.json') as json_file:
        data_dict = json.load(json_file)
        print(data_dict)

    for k, v in reversed(data_dict.items()):
        for val in v:
            results.append([k, val, 'vanilla-ipc-16MB'])

    with open('rados-ipc-16.json') as json_file:
        data_dict = json.load(json_file)
        print(data_dict)

    for k, v in reversed(data_dict.items()):
        for val in v:
            results.append([k, val, 'rados-ipc-16MB'])
    ...
    plt.tight_layout()
    plt.savefig(f"rados-pq-vs-ipc-16MB.pdf", bbox_inches='tight')
